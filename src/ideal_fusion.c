...
/* I-Fuse ideal fusion: gmon -> Op* (with unique_num guard) table.
 * Populated at LOAD1's update_map, looked up at LOAD2's update_map to
 * redirect reg_map.op. */
#define IFUSE_REMAP_HT_SIZE 1000003

typedef struct IfuseRemapEntry {
  unsigned int            load1_gmon;
  Op*                     load1_op;
  Counter                 load1_unique_num;
  struct IfuseRemapEntry* next;
} IfuseRemapEntry;

static IfuseRemapEntry* ifuse_remap_ht[IFUSE_REMAP_HT_SIZE];

void ifuse_remap_record_load1(unsigned int load1_gmon, Op* op, Counter unique_num) {
  unsigned int idx = (unsigned int)((load1_gmon * 2654435761u) % IFUSE_REMAP_HT_SIZE);
  for (IfuseRemapEntry* e = ifuse_remap_ht[idx]; e; e = e->next) {
    if (e->load1_gmon == load1_gmon) {
      e->load1_op         = op;
      e->load1_unique_num = unique_num;
      return;
    }
  }
  IfuseRemapEntry* e = (IfuseRemapEntry*)malloc(sizeof(IfuseRemapEntry));
  if (!e) return;
  e->load1_gmon       = load1_gmon;
  e->load1_op         = op;
  e->load1_unique_num = unique_num;
  e->next             = ifuse_remap_ht[idx];
  ifuse_remap_ht[idx] = e;
}
...
Op* ifuse_remap_lookup_load1_op(unsigned int load1_gmon, Counter* out_unique_num) {
  ...
  for (IfuseRemapEntry* e = ifuse_remap_ht[idx]; e; e = e->next) {
    if (e->load1_gmon == load1_gmon) {
      if (out_unique_num) *out_unique_num = e->load1_unique_num;
      return e->load1_op;
    }
  }
  return NULL;
...
static void load_fusion_candidates(void) {
  ...
  for (int i = 0; i < IFUSE_REMAP_HT_SIZE; i++) {
    ifuse_remap_ht[i] = NULL;
  }
